import { K as f, L as s, B as z, j as w, k as I, N as D, D as S, f as k, h as u, U as p, S as b, m as g } from "./vendor-audio-DkF9nV6x.js";
import { I as C } from "./ID3v2Parser-Boz_HjH8.js";
import "./vendor-react-C6Febm84.js";
import "./vendor-i18n-Knli4vfm.js";
import "./vendor-ui-BG0coi97.js";
import "./vendor-uppy-DElw6E-D.js";
const o = { len: 12, get: (h, t) => ({ chunkID: s.get(h, t), chunkSize: f.get(h, t + 4) }) }, a = w("music-metadata:parser:aiff");
...
class N extends z {
  async parse() {
    ...
    if (e === "DSD") return this.metadata.setFormat("container", `DSDIFF/${e}`), this.metadata.setFormat("lossless", true), this.readFmt8Chunks(t.chunkSize - BigInt(s.len));
  ...
  async readData(t) {
    ...
    switch (t.chunkID.trim()) {
      case "FVER": {
        const r = await this.tokenizer.readToken(S);
        a(`DSDIFF version=${r}`);
        break;
      }
      case "PROP": {
      ...
      case "ID3": {
        const r = await this.tokenizer.readToken(new I(Number(t.chunkSize))), n = D(r);
    ...
    for (a(`Parsing sound-property-chunks, remainingSize=${t}`); t > 0; ) {
      ...
      switch (e.chunkID.trim()) {
        ...
        case "CHNL": {
          ...
          this.metadata.setFormat("numberOfChannels", n), await this.handleChannelChunks(e.chunkSize - BigInt(k.len));
          break;
        }
        case "CMPR": {
          const n = (await this.tokenizer.readToken(s)).trim(), c = await this.tokenizer.readToken(u), m = await this.tokenizer.readToken(new b(c, "ascii"));
          n === "DSD" && (this.metadata.setFormat("lossless", true), this.metadata.setFormat("bitsPerSample", 1)), this.metadata.setFormat("codec", `${n} (${m})`);
          break;
        }
        case "ABSS": {
          const n = await this.tokenizer.readToken(k), c = await this.tokenizer.readToken(u), m = await this.tokenizer.readToken(u), l = await this.tokenizer.readToken(p);
          a(`ABSS ${n}:${c}:${m}.${l}`);
          break;
        }
        case "LSCO": {
