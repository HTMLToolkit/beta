...
class U {
  ...
  async parseContainer(a, t, u) {
    ...
    for (; this.tokenizer.position < t; ) {
      ...
      if (n) switch (u.startNext(n)) {
        case c.ReadNext:
          if (i.id, g(`Read element: name=${x(n)}{id=0x${i.id.toString(16)}, container=${!!n.container}} at position=${s}`), n.container) {
            const m = await this.parseContainer(n, i.len >= 0 ? this.tokenizer.position + i.len : -1, u);
            n.multiple ? (o[n.name] || (o[n.name] = []), o[n.name].push(m)) : o[n.name] = m, await u.elementValue(n, m, s);
          } else {
            const m = this.parserMap.get(n.value);
            if (typeof m == "function") {
              const d = await m(i);
              o[n.name] = d, await u.elementValue(n, d, s);
            }
          }
          break;
        case c.SkipElement:
          g(`Go to next element: name=${x(n)}, element.id=0x${i.id}, container=${!!n.container} at position=${s}`);
          break;
        case c.IgnoreElement:
          g(`Ignore element: name=${x(n)}, element.id=0x${i.id}, container=${!!n.container} at position=${s}`), await this.tokenizer.ignore(i.len);
          break;
        case c.SkipSiblings:
          g(`Ignore remaining container, at: name=${x(n)}, element.id=0x${i.id}, container=${!!n.container} at position=${s}`), await this.tokenizer.ignore(t - this.tokenizer.position);
          break;
        case c.TerminateParsing:
          return g(`Terminate parsing at element: name=${x(n)}, element.id=0x${i.id}, container=${!!n.container} at position=${s}`), o;
      }
      else i.id === 236 ? (this.padding += i.len, await this.tokenizer.ignore(i.len)) : (g(`parseEbml: parent=${x(a)}, unknown child: id=${i.id.toString(16)} at position=${s}`), this.padding += i.len, await this.tokenizer.ignore(i.len));
    }
    return o;
  }
  async readVintData(a) {
    ...
    let u = 128, o = 1;
    for (; (t & u) === 0; ) {
      if (o > a) throw new v("VINT value exceeding maximum size");
      ++o, u >>= 1;
    }
...
class H extends z {
  constructor() {
    super(...arguments), this.seekHeadOffset = 0, this.flagUseIndexToSkipClusters = this.options.mkvUseIndex ?? false;
  }
  async parse() {
    const a = this.tokenizer.fileInfo.size ?? Number.MAX_SAFE_INTEGER, t = new U(this.tokenizer);
    p("Initializing DTD end MatroskaIterator"), await t.iterate(F, a, { startNext: (u) => {
      switch (u.id) {
        case 475249515:
          return p(`Skip element: name=${u.name}, id=0x${u.id.toString(16)}`), c.IgnoreElement;
        case 524531317:
          if (this.flagUseIndexToSkipClusters && this.seekHead) {
            const o = this.seekHead.seek.find((i) => i.position + this.seekHeadOffset > this.tokenizer.position);
            if (o) {
              const i = o.position + this.seekHeadOffset - this.tokenizer.position;
              return p(`Use index to go to next position, ignoring ${i} bytes`), this.tokenizer.ignore(i), c.SkipElement;
            }
          }
    ...
    }, elementValue: async (u, o, i) => {
      switch (p(`Received: name=${u.name}, value=${o}`), u.id) {
        case 17026:
          this.metadata.setFormat("container", `EBML/${o}`);
          break;
        case 290298740:
          this.seekHead = o, this.seekHeadOffset = i;
          break;
        case 357149030:
          {
            const s = o, n = s.timecodeScale ? s.timecodeScale : 1e6;
            if (typeof s.duration == "number") {
              const r = s.duration * n / 1e9;
        ...
        case 374648427:
          {
            const s = o;
            if (s == null ? void 0 : s.entries) {
              s.entries.forEach((r) => {
                const m = { codecName: r.codecID.replace("A_", "").replace("V_", ""), codecSettings: r.codecSettings, flagDefault: r.flagDefault, flagLacing: r.flagLacing, flagEnabled: r.flagEnabled, language: r.language, name: r.name, type: r.trackType, audio: r.audio, video: r.video };
                this.metadata.addStreamInfo(m);
              });
              const n = s.entries.filter((r) => r.trackType === N.audio).reduce((r, m) => !r || m.flagDefault && !r.flagDefault || m.trackNumber < r.trackNumber ? m : r, null);
        ...
        case 307544935:
          {
            ...
            await Promise.all(s.tag.map(async (n) => {
              const r = n.target, m = (r == null ? void 0 : r.targetTypeValue) ? D[r.targetTypeValue] : (r == null ? void 0 : r.targetType) ? r.targetType : "track";
        ...
        case 423732329:
          {
            const s = o;
            await Promise.all(s.attachedFiles.filter((n) => n.mimeType.startsWith("image/")).map((n) => this.addTag("picture", { data: n.data, format: n.mimeType, description: n.description, name: n.name })));
          }
          break;
      }
    } });
