import { A as R, U as o, S as w, h as E, H as m, z as v, j as U, m as A, C as g, g as p, w as N, k as I, f as k, O as M } from "./vendor-audio-DrJdxnoN.js";
import { A as x } from "./AbstractID3Parser-B9quUGST.js";
class l {
  static fromBase64(t) {
  ...
  get(t, e) {
    ...
    return { type: r, format: i, description: T, width: C, height: P, colour_depth: L, indexed_color: b, data: O };
  }
}
const d = { len: 7, get: (a, t) => ({ packetType: E.get(a, t), vorbis: new w(6, "ascii").get(a, t + 1) }) }, _ = { len: 23, get: (a, t) => ({ version: m.get(a, t + 0), channelMode: E.get(a, t + 4), sampleRate: m.get(a, t + 5), bitrateMax: m.get(a, t + 9), bitrateNominal: m.get(a, t + 13), bitrateMin: m.get(a, t + 17) }) };
class B {
  constructor(t, e) {
  ...
  parseUserComment() {
    ...
    return { key: e.substring(0, r).toUpperCase(), value: e.substring(r + 1), len: this.offset - t };
  }
}
const h = U("music-metadata:parser:ogg:vorbis1");
class y extends A("Vorbis") {
}
class u {
  constructor(t, e) {
    this.pageSegments = [], this.durationOnLastPage = true, this.metadata = t, this.options = e;
  }
  async parsePage(t, e) {
  ...
  async addTag(t, e) {
    if (t === "METADATA_BLOCK_PICTURE" && typeof e == "string") {
      ...
      e = l.fromBase64(e), h(`Push picture: id=${t}, format=${e.format}`);
    } else h(`Push tag: id=${t}, value=${e}`);
  ...
  parseFirstPage(t, e) {
    this.metadata.setFormat("codec", "Vorbis I"), this.metadata.setFormat("hasAudio", true), h("Parse first page");
    const r = d.get(e, 0);
    if (r.vorbis !== "vorbis") throw new y("Metadata does not look like Vorbis");
    if (r.packetType === 1) {
      const s = _.get(e, d.len);
      this.metadata.setFormat("sampleRate", s.sampleRate), this.metadata.setFormat("bitrate", s.bitrateNominal), this.metadata.setFormat("numberOfChannels", s.channelMode), h("sample-rate=%s[hz], bitrate=%s[b/s], channel-mode=%s", s.sampleRate, s.bitrateNominal, s.channelMode);
    } else throw new y("First Ogg page should be type 1: the identification header");
  }
  async parseFullPage(t) {
    const e = d.get(t, 0);
    if (h("Parse full page: type=%s, byteLength=%s", e.packetType, t.byteLength), e.packetType === 3) return this.parseUserCommentList(t, d.len);
  }
...
const c = { STREAMINFO: 0, PADDING: 1, APPLICATION: 2, SEEKTABLE: 3, VORBIS_COMMENT: 4, CUESHEET: 5, PICTURE: 6 }, D = { len: 4, get: (a, t) => ({ lastBlock: N(a, t, 7), type: p(a, t, 1, 7), length: g.get(a, t + 1) }) }, S = { len: 34, get: (a, t) => ({ minimumBlockSize: k.get(a, t), maximumBlockSize: k.get(a, t + 2) / 1e3, minimumFrameSize: g.get(a, t + 4), maximumFrameSize: g.get(a, t + 7), sampleRate: g.get(a, t + 10) >> 4, channels: p(a, t + 12, 4, 3) + 1, bitsPerSample: p(a, t + 12, 7, 5) + 1, totalSamples: p(a, t + 13, 4, 36), fileMD5: new I(16).get(a, t + 18) }) }, $ = U("music-metadata:parser:FLAC");
...
class H extends x {
  ...
  async postId3v2Parse() {
    if ((await this.tokenizer.readToken(M)).toString() !== "fLaC") throw new F("Invalid FLAC preamble");
    let e;
    do
  ...
  async readBlockStreamInfo(t) {
    if (t !== S.len) throw new F("Unexpected block-stream-info length");
  ...
  processsStreamInfo(t) {
    this.metadata.setFormat("codec", "FLAC"), this.metadata.setFormat("hasAudio", true), this.metadata.setFormat("lossless", true), this.metadata.setFormat("numberOfChannels", t.channels), this.metadata.setFormat("bitsPerSample", t.bitsPerSample), this.metadata.setFormat("sampleRate", t.sampleRate), t.totalSamples > 0 && this.metadata.setFormat("duration", t.totalSamples / t.sampleRate);
  }
  async readComment(t) {
  ...
  async parseComment(t) {
    ...
    for (let n = 0; n < s; n++) i[n] = e.parseUserComment();
...
const K = Object.freeze(Object.defineProperty({ __proto__: null, FlacParser: H }, Symbol.toStringTag, { value: "Module" }));
export {
  D as B,
  H as F,
  u as V,
